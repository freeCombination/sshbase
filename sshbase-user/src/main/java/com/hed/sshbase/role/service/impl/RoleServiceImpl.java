package com.hed.sshbase.role.service.impl;
...
import com.hed.sshbase.role.entity.Role;
...
import com.hed.sshbase.role.vo.RoleVo;
import com.hed.sshbase.user.entity.Group;
import com.hed.sshbase.user.entity.User;
import com.hed.sshbase.user.util.HSSFUtils;
import com.hed.sshbase.user.vo.GroupVo;
...
public class RoleServiceImpl implements IRoleService {
    ...
    public String deleteRole(String roleId)
        ...
        try {
            if (StringUtils.isNotBlank(roleId)) {
                ...
                if (!CollectionUtils.isEmpty(rmss)) {
    ...
    public PagerVo<Role> getRoleList(Role role, int start, int limit)
        ...
        if (role != null) {
            if (StringUtils.isNotBlank(role.getRoleName())) {
                hql.append(" and r.roleName like :rname");
                
                values.put("rname", "%" + role.getRoleName() + "%");
            }
        }
        hql.append(" order by r.roleId ");
        List<Role> userList =
            (List<Role>)baseDao.queryEntitysByPage(start, limit, hql.toString(), values);
    ...
    public int getRoleNumByName(String roleName, String roleType)
    	...
    	if(StringUtils.isNotBlank(roleName)){
    	    String sql =
    	        " FROM Role a WHERE a.isDelete = "
    	            + Constant.STATUS_NOT_DELETE + " AND a.roleType.pkDictionaryId = "
    	            + roleType + " and a.roleName=:rname";
    	    
    	    Map<String, Object> values = new HashMap<String, Object>();
    	    values.put("rname", roleName);
    	    
    	    count = baseDao.queryTotalCount(sql, values);
    	}
        return count;
    }
    ...
    public void deleteRoleResources(String resourceIds, String roleId)
        ...
        for (String resourceId : resourceIds.split(",")) {
            ...
            String deleteSql =
                "DELETE FROM T_ROLE_RESOURCE WHERE resource_id=? and  role_id =?";
            this.baseDao.executeNativeSQL(deleteSql, new Object[] {resourceId, roleId});
        }
    }
    ...
    public int validateAddRoleCode(String roleCode, String roleType)
        throws BusinessException {
        int count = 0;
        String sql = " FROM Role a WHERE a.isDelete = " + Constant.STATUS_NOT_DELETE 
            + " AND a.roleType.pkDictionaryId = :rtype AND  a.roleCode=:rcode";
        
        Map<String, Object> param = new HashMap<String, Object>();
        param.put("rtype", roleType);
        param.put("rcode", roleCode);
        
        count = this.baseDao.queryTotalCount(sql, param);
    ...
    public int validateUpdateRoleName(String roleName, String oldRoleId,
        String roleType)
        throws BusinessException {
        Role oldRole = (Role)this.baseDao.queryEntityById(Role.class,
                NumberUtils.toInt(oldRoleId));
       
        String sql = " FROM Role a WHERE a.isDelete = " + Constant.STATUS_NOT_DELETE
            + " AND a.roleType.pkDictionaryId = :rtype"
            + " and a.roleName= :rname1"
            + " and a.roleName != :rname2";
        
        Map<String, Object> param = new HashMap<String, Object>();
        param.put("rtype", roleType);
        param.put("rname1", roleName);
        param.put("rname2", oldRole.getRoleName());
        
        return this.baseDao.queryTotalCount(sql, param);
    }
    ...
    public int validateUpdateRoleCode(String roleCode, String oldRoleId,
        ...
        String sql = "FROM Role a WHERE a.isDelete = " + Constant.STATUS_NOT_DELETE
            + " AND a.roleType.pkDictionaryId = :rtype"
            + " AND  a.roleCode = :rcode1"
            + " and ROLE_CODE != :rcode2";
        
        Map<String, Object> param = new HashMap<String, Object>();
        param.put("rtype", roleType);
        param.put("rcode1", roleCode);
        param.put("rcode2", oldRole.getRoleCode());
    ...
    public ListVo<Role> getRoleListByRoleName(String roleName, int pageIndex,
        ...
        if (!"".equals(roleName) && roleName != null) {
            hql += " and r.roleName like :rname";
            
            param.put("rname", "%" + roleName + "%");
        }
        
        List<Role> roleList = baseDao.queryEntitysByMap(hql, param);
        int count =
            baseDao.getTotalCount("select count(1) " + hql, param);
    ...
    public Role getLocalRoleByName(String roleName)
        throws BusinessException {
        String hql =
            "from Role r where r.roleName = :rname"
    ...
    public ListVo<GroupVo> getRoleMember(Map<String, String> param)
        ...
        if (StringUtils.isNotBlank(groupName)) {
            hql.append(" and rms.group.groupName like :gpname");
            hqlCount.append(" and rms.group.groupName like :gpname");
            
            values.put("gpname", groupName);
        }
        ...
        List<RoleMemberScope> rms =
            (List<RoleMemberScope>)baseDao.queryEntitysByPage(NumberUtils.toInt(start),
                NumberUtils.toInt(limit), hql.toString(), values);
        ...
        int count = baseDao.getTotalCount(hqlCount.toString(), values);
        vos.setList(wr);
    ...
    public ListVo<UserVo> getRoleMemberByUser(Map<String, String> param)
        ...
        if (StringUtils.isNotBlank(realName)) {
            hql.append(" and rms.user.realname like :rname");
            hqlCount.append(" and rms.user.realname like :rname");
            
            values.put("rname", "%" + realName + "%");
        }
    ...
    public ResponseVo getRoleListByUser(Map<String, String> param)
        throws BusinessException {
        // 获取参数
        String start = (String)param.get("start");
        String limit = (String)param.get("limit");
        ...
        hql.append("select role from Role role,RoleMemberScope rms where rms.role=role and rms.user.userId =  "+userId);
        hqlCount.append("select count(*) from Role role,RoleMemberScope rms where rms.role=role and rms.user.userId =  "+userId);
        
        List<Role> rms =
            (List<Role>)baseDao.queryEntitysByPage(NumberUtils.toInt(start),
                NumberUtils.toInt(limit),
                hql.toString(),
                new HashMap<String, Object>());
        RoleVo roleVo;
        List<RoleVo> roList = new ArrayList<RoleVo>();
        for(Role role : rms){
        ...
        int count =
            baseDao.getTotalCount(hqlCount.toString(),
                new HashMap<String, Object>());
        return new ResponseVo(roList,count);
    }
    ...
    public ListVo<UserVo> getScopeByMenberId(Map<String, String> param) throws BusinessException {
        ...
        if (StringUtils.isNotBlank(realname)) {
            hql.append(" and u.realname like :rname");
            hqlCount.append(" and u.realname like :rname");

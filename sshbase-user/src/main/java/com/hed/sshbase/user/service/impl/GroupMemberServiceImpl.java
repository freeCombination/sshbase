...
import com.hed.sshbase.user.service.IGroupMemberService;
import com.hed.sshbase.user.service.IUserService;
import com.hed.sshbase.user.vo.GroupMemberVo;
...
public class GroupMemberServiceImpl implements IGroupMemberService {
    ...
    public String addGroupMember(Map<String, String> param)
        ...
        try {
            ...
            if (StringUtils.isNotBlank(groupId) && StringUtils.isNotBlank(userIds)) {
                ...
                List<GroupMember> gmLst = new ArrayList<GroupMember>();
                for (String userId : strs) {
                    ...
                    if (CollectionUtils.isEmpty(gm)) {// 说明此用户还没有存在
                        groupMember = new GroupMember();
                    }
                    else {
                        groupMember = gm.get(0);
                    }
                    
                    User user = (User)baseDao.queryEntityById(User.class, NumberUtils.toInt(userId));
                    groupMember.setUser(user);
                    // 设置群组
                    Group group = (Group)baseDao.queryEntityById(Group.class, NumberUtils.toInt(groupId));
                    groupMember.setGroup(group);
                    
                    gmLst.add(groupMember);
                }
                
                baseDao.saveOrUpdate(gmLst);
                result = "success";
            }
        }
        catch (Exception e) {
    ...
    public ListVo<GroupMemberVo> getGroupMember(Map<String, String> paramMap)
        ...
        if (StringUtils.isNotBlank(realname)) {
            hql.append(" and o.user.realname like :rname");
            
            param.put("rname", "%" + realname + "%");
        }
        ...
        try {
            // 统计中条数
            totalSize = baseDao.queryTotalCount(hql.toString(), param);
            List<GroupMember> list = baseDao.queryEntitysByPage(NumberUtils.toInt(start),
                NumberUtils.toInt(limit), hql.toString(), param);
    ...
    public ListVo<GroupMemberVo> getDept(Map<String, String> paramMap)
        ...
        if (StringUtils.isNotBlank(orgName)) {
            hql.append(" and o.org.orgName like :oname");
            
            param.put("oname", "%" + orgName + "%");
        }
        ...
        try {
            ...
            list =
                (List<GroupMember>)baseDao.queryEntitysByPage(Integer.parseInt(start),
                    Integer.parseInt(limit), hql.toString(), param);
            // 用于存储群组用户VO的集合
            List<GroupMemberVo> memberVos = new ArrayList<GroupMemberVo>();
            GroupMemberVo groupMemberVo = null;
            // 循环遍历结果集，封装对象并添加群组成员VO集合
    ...
    public ListVo<UserVo> getUserList(Map<String, String> paramMap) 
        ...
        String orgIds = orgService.getOrgIdsByPermissionScope(orgId, null);
        
        // 用户存储hql语句
        StringBuffer hql = new StringBuffer();
        hql.append(" select u from User u, OrgUser ou where u.isDeletAble=" + Constant.ALLOW_DELETE);
        hql.append(" and ou.isDelete=" + Constant.STATUS_NOT_DELETE);
        hql.append(" and u.status=" + Constant.STATUS_NOT_DELETE);
        hql.append(" and ou.organization.status=" + Constant.STATUS_NOT_DELETE);
        hql.append(" and ou.user.userId = u.userId ");
        ...
        if (StringUtils.isNotBlank(userName)) {
            hql.append(" and u.realname like :uname");
            
            param.put("uname", "%" + userName + "%");
        }
        
        if (StringUtils.isNotBlank(groupId)) {
            hql.append(" and u.userId not in (select o.user.userId from GroupMember o where o.user is not null");
            hql.append(" and o.group.id=" + groupId + ")");
        }
        ...
        int totalSize = baseDao.queryTotalCount(hql.toString(), param);
        
        hql.append(" order by u.disOrder, u.userId");
        List<User> userList =
            (List<User>)baseDao.queryEntitysByPage(start, limit, hql.toString(), param);

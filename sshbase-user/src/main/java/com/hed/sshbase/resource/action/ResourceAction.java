package com.hed.sshbase.resource.action;
...
public class ResourceAction extends BaseAction {
    ...
    public String getResourceList() {
       //优化之前的代码
    	...
    	ListVo<ResourceVo> list;
        try {
            list = resourceService.getResourceList(map);
     ...
     * @return
     */
    public String getResourceListByMap() {
    	Map<String, String> map = RequestUtil.getParameterMap(getRequest());
    	ResponseVo rv = new ResponseVo();
    	ListVo<ResourceVo> list;
        try {
     ...
     * 得到同步资源树
     * 
     * @Title getSynchResTree
     * @author wanglc
     * @date 2014-3-11
     * @return
     */
    /*public String getSynchResTree() {
        // TODO 优化
        try {
            ...
            List<Integer> resIdsByRole = new ArrayList<Integer>();
            List<Integer> resIdsByUser = new ArrayList<Integer>();
            resList = new ArrayList<Resource>();
            
            // 用户资源
            List<UserOrgResource> ownedReslistByUser =
                resourceService.getOwnResByUserAndOrg(userId, orgIds);
            if (ownedReslistByUser != null) {
                for (UserOrgResource rr : ownedReslistByUser) {
                    resIdsByUser.add(rr.getResource().getResourceId());
                }
            }
            User u = userService.getUserById(NumberUtils.toInt(userId));
            if (u != null) {
                List<UserRole> urList =
                    userRoleService.getUserRoleListByUser(u);
                if (urList.size() > 0) {
                    String roleIds = "";
                    for (UserRole ur : urList) {
                        roleIds += (ur.getRole().getRoleId() + ",");
                    }
                    // 角色资源
                    List<RoleResource> ownedReslistByRole =
                        roleResourceService.getOwnResByUserRole(roleIds.substring(0,
                            roleIds.length() - 1));
                    if (ownedReslistByRole != null) {
                        for (RoleResource rr : ownedReslistByRole) {
                            resIdsByRole.add(rr.getResource().getResourceId());
                        }
                    }
                }
            }
            List<Resource> res = resourceService.getFirstLevelResource();
            ...
            this.excepAndLogHandle(ResourceAction.class, "得到同步资源树", e, false);
        }
        return null;
    }*/
     ...
     * 构造资源树
     * 
     * @Title getResourceTreeList
     * @author wanglc
     * @date 2013-11-25
     * @return
     */
    
    /*public String getResourceTreeList() {
        // TODO 优化
        try {
            ...
            if ("0".equals(parentId)) {
                resList = resourceService.getFirstLevelResource();
            } else {
                resList = resourceService.getAllSonResByParentId(parentId);
            }
            // 用户资源
            ...
            for (Resource r : resList) {
                ...
                tn.setLeaf(decideIsLeaf(r));
                tn.setChecked(isChecked(resIdsByUser, resIdsByRole, r));
                treeNodeList.add(tn);
            }
            this.excepAndLogHandle(ResourceAction.class, "构造资源树", null, true);
        } catch (Exception e) {
            this.excepAndLogHandle(ResourceAction.class, "构造资源树", e, false);
        }
        return SUCCESS;
    }*/
     ...
     * 同步集成平台的资源数据到本地
     * 
     * @Title synchronizeResourceInfo
     * @author wanglc
     * @date 2013-12-17
     * @return
     */
    public String getResourceListByRoleIds() {
    	ResponseVo rv;
    	try {
    		Map<String, String> paramsMap =  RequestUtil.getParameterMap(getRequest());
    		String id = paramsMap.get("id");
    		if(StringUtils.isNotBlank(id)){
    			rv = resourceService.getResourceListByRoleIds(super.getSession().getAttribute("roleIds").toString(), id);
    		}else{
    			rv = new ResponseVo("请重新登录，或者当前用户无可用角色");
    		}
    		JsonUtil.outJson(rv);
    	} catch (BusinessException e) {
    		this.excepAndLogHandle(ResourceAction.class,

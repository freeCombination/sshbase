package com.hed.sshbase.resource.service.impl;
...
public class ResourceServiceImpl implements IResourceService {
    ...
    public ListVo<ResourceVo> getResourceListRecord(Map<String, String> map)
        ...
        StringBuffer sql = new StringBuffer( "From Resource a where a.status = "
        ...
        if (!StringUtils.isBlank(resourceName)) {
            sql.append(" and a.resourceName like :rsname");
            
            values.put("rsname", "%" + resourceName + "%");
        }
        if (hqBf.length()>0) {
            sql.append(" and a.resourceId in( ").append(hqBf.substring(1)).append(")");
        }
        
        int totalSize =  baseDao.queryTotalCount(sql.toString(), values);
        
        sql.append(" order by a.resourceType.pkDictionaryId,a.resource.resourceId,a.disOrder, a.resourceId ");
        List<Resource> result = baseDao.queryEntitysByPage(start, limit, sql.toString(), values);
    ...
    private List<Resource>  getResListByHierarchicalQuery(int startId, List<Resource> listResult)
            ...
            // 判断组织id是否为空
                ...
                Resource res =(Resource) baseDao.queryEntityById(Resource.class,startId);
               //listResult.add(res);
    ...
    public ListVo<ResourceVo> getResourceList(Map<String, String> map)
        ...
        Resource res = null;
        StringBuffer hqlQuery = new StringBuffer("select r from Resource r where r.status=0 ");
        if (!StringUtils.isBlank(resourceName)) {
        ...
        if (StringUtils.isNotBlank(resourceType)) {
            hqlQuery.append(" and r.resourceType.dictCode = :dictCode ");
            param.put("dictCode", resourceType);
        }
        ...
        int totalSize = this.baseDao.queryTotalCount(hqlQuery.toString(), param);
        
        hqlQuery.append(" order by r.disOrder,r.resourceId desc ");
        List<Resource> resourceList = (List<Resource>)this.baseDao.queryEntitysByPage(
            start,limit,hqlQuery.toString(),param);
    ...
    public boolean validateUpdateResourceName(String resourceName,
        ...
        String sql =
            "SELECT COUNT(RESOURCE_ID) FROM Resource a WHERE a.status = " + Constant.STATUS_NOT_DELETE
            + " AND a.resourceName=:rsname and a.resourceName != :rsname1" ;
        
        Map<String, Object> param = new HashMap<String, Object>();
        param.put("rsname", resourceName);
        param.put("rsname1", oldResource.getResourceName());
        
        int flag = baseDao.queryTotalCount(sql, param);
        if (flag == 0) {
            return true;
        } else {
            return false;
    ...
    public boolean isLeaf(int resourceId)
        ...
        String hql =
            "from Resource r where r.status=" + Constant.STATUS_NOT_DELETE
                + " and r.resourceType.dictCode='PAGE' and r.resource.resourceId = " + resourceId;
     ...
     */
    @Override
    public PagerVo<Resource> getResourceList(int start, int limit, Resource resource)
        throws BusinessException {
        StringBuffer hql =
        ...
        if (resource != null) {
            if (StringUtils.isNotBlank(resource.getResourceName())) {
                hql.append(" and r.resourceName like :rsname"
                    + resource.getResourceName() + "%' ");
                
                values.put("rsname", "%" + resource.getResourceName() + "%");
            }
            if (StringUtils.isNotBlank(resource.getResourceType().getDictionaryName())) {
                hql.append(" and r.resourceType.dictionaryName = :dictname");
                
                values.put("dictname", resource.getResourceType().getDictionaryName());
            }
     ...
     * @return List<Resource>
     */
    public List<Resource> getResourceListByRoleAndType(String roleIds,String resourceTypeCode)
        ...
        String hql =
            "select distinct rr from Resource rr,RoleResource t where 1=1  "
                + " and rr.resourceId=t.resource.resourceId and rr.status=0 "
                + " and  t.role.roleId in(" + roleIds
                + ") ";
        if(StringUtils.isNotBlank(resourceTypeCode)){
        	hql+= "  and rr.resourceType.dictCode='"+resourceTypeCode+"' ";
        }
        	hql+= "order by rr.disOrder";
	...
	public ResponseVo getResourceListByRoleIds(String ids,String parentCode)
			throws BusinessException {
		String hql = "select distinct rr.resource from RoleResource rr  where rr.resource.resource.resourceId="+parentCode+" and rr.role.roleId in (" + ids
				+ ")";
		List<Resource> rrList = baseDao.queryEntitys(hql);
		List<ResourceVo> resourceVoList = new ArrayList<ResourceVo>();
		for (int i = 0; i < rrList.size(); i++) {
            Resource r = rrList.get(i);
            ResourceVo rV = new ResourceVo(r);
            resourceVoList.add(rV);
        }
		return new ResponseVo(resourceVoList,resourceVoList.size());
	}
}

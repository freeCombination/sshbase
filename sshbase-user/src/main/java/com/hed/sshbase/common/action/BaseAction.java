package com.hed.sshbase.common.action;
       ...
       // log.setSrcTableName("srcTalble");
        ...
        if (flag) {
            ...
            log.setOpContent(getCurrentUser().getUsername() + " 执行【" + msg
                + "】功能，正常！");
            log.setType(new Dictionary(Constant.SYS_LOG));
        }
        // flag = false 异常日志信息
        else {
            ...
            log.setOpContent(getCurrentUser().getUsername() + " 执行【" + msg
                + "】功能，发生异常：" + ex);
            log.setType(new Dictionary(Constant.ERR_LOG));
        }
        try {

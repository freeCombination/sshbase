package com.hed.sshbase.dict.action;
...
public class DictAction extends BaseAction {
     ...
     * @return 字典对象
     */
    public Dictionary getDictById() {
    	String id = getRequest().getParameter("id");
    	Dictionary dict ;
    	try {
    		dict = dictService.getDictById(NumberUtils.toInt(id));
    		DictionaryVo dv = new DictionaryVo();
    		dv.setDictionaryName(dict.getDictionaryName());
    		dv.setPkDictionaryId(dict.getPkDictionaryId());
    		JsonUtil.outJson(new ResponseVo(dv));
    	}
    	catch (BusinessException e) {
    		// 发生异常是，进行日志入库和生成日志文件
    		this.excepAndLogHandle(DictAction.class, "通过id查询字典数据", e, false);

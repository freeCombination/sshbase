package com.hed.sshbase.org.service.impl;
...
import com.hed.sshbase.common.util.ConnectionFactory;
import com.hed.sshbase.common.util.PropertyUtil;
...
public class OrgServiceImpl implements IOrgService {
    ...
    public boolean checkOrgName(Organization o, String orgName)
        ...
        if (StringUtils.isNotBlank(orgName)) {
            ...
            Map<String, Object> values = new HashMap<String, Object>();
            values.put("oname", orgName);
            
            int count = baseDao.getTotalCount(sql, values); 
            if (count == 0) {
                return true;
            }
        }
    ...
    public boolean validateAddOrgName(String orgName)
        throws BusinessException {
        String sql = "SELECT COUNT(*) FROM Organization a WHERE a.orgName=:oname";
        
        Map<String, Object> values = new HashMap<String, Object>();
        values.put("oname", orgName);
        
        int flag =baseDao.getTotalCount(sql, values);
    ...
    public Organization getOrganizationByCode(String code)
        throws BusinessException {
        String hql = "from Organization o where o.enable = " + Constant.ENABLE
            + " and o.status = " + Constant.STATUS_NOT_DELETE
            + " and o.orgCode = :ocode order by o.disOrder";
        
        Map<String, Object> values = new HashMap<String, Object>();
        values.put("ocode", code);
        
        List<Organization> ul =
            (List<Organization>)this.baseDao.queryEntitysByMap(hql, values);
    ...
    private Organization getLocalOrgByCode(String code) {
        List<Organization> orgList = new ArrayList<Organization>();
        String hql = "from Organization o where o.orgCode = :ocode"
                + " and o.status = " + Constant.STATUS_NOT_DELETE;
        
        Map<String, Object> values = new HashMap<String, Object>();
        values.put("ocode", code);
        
        orgList = baseDao.queryEntitysByMap(hql, values);
    ...
    public ArrayList<TreeNode> getUnitTreeListForModifyUser(
        ...
        String orgTypeCode = StringUtil.getStr(params.get("orgTypeCode"));
     ...
     */
    @Override
    public ResponseVo getOrgTreeNodeList(Map<String, String> params) throws BusinessException {
		...
		String hql = "from Organization o where o.enable=" + Constant.ENABLE
				+ " and o.status=" + Constant.STATUS_NOT_DELETE;
		if ("0".equals(parentId)) {
			hql += " and (o.organization.orgId= 0 or o.organization is null) ";
		}
        if(StringUtils.isNotBlank(orgTypeCode)){
        	 hql +=  " and o.orgType.dictCode = '"+orgTypeCode+"' ";
        }
        hql += " order by o.disOrder";
        List<Organization> result = baseDao.queryEntitys(hql);
        ...
        for (Organization o : result) {
            ...
            if(o.getOrganization()!=null){
            	tn.setParentId(o.getOrganization().getOrgId());
            }
            tn.setNodeId(o.getOrgId());
            tn.setText(o.getOrgName());
            tn.setLeaf(organizationService.isLeafOrg(o));
    ...
    public ListVo<OrgVo> getOrgList(Map<String, String> map)
        ...
        String orgIds="";
        if(StringUtil.isBlank(parentOrgId)){
        	parentOrgId="0";
        }
        orgIds = getOrgIdsByPermissionScope(parentOrgId, null);
        int totalSize = 0;

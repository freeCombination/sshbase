...
<body class="overflow_hidden" onload="getIE();">
	<div class="frame2_top">
		<div class="top_logo">海关实时数据监控系统</div>
		<div class="top_img">
		...
		<ul>
			<s:iterator value="menuVos"  status="s">
				<li style="cursor: pointer;" id="topLi_<s:property value="#s.index"/>" <s:if test="children!=null">  onmouseover="topHover_show(<s:property value="#s.index"/>)" onmouseout="topHover_unShow(<s:property value="#s.index"/>)"</s:if>>
					...
					<!-- <a href="${ctx}/toCenter.action?id=<s:property value="id"/>" onclick="getBg(<s:property value="#s.index"/>)" url="<s:property value="url"/>" target="frame_Center" ><s:property value="name"/></a> -->
					<a href="${ctx}/<s:property value="url"/>" onclick="getBg(<s:property value="#s.index"/>)" url="<s:property value="url"/>" target="frame_Center" ><s:property value="name"/></a>
					</s:if>
					<s:else>
					...
					<s:iterator value="children">
						<span><a href="${ctx}/<s:property value="url"/>" onclick="getBg(<s:property value="#s.index"/>)" target="frame_Center" ><s:property value="name"/></a></span>
					</s:iterator>
					</div>
					</s:if>
				</li>
